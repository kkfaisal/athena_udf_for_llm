...
def process_records(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        return [
            future.result()
            for future in as_completed([executor.submit(func, *args, record) for record in records]) #list(record.values()) -> record
        ]
...
def process_records_in_chunks(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            [executor.submit(func, *args, record) for record in batch] #list(record.values()) -> record
